...
export type HeaderNavModulesConfig = {
  ...
  modelHealth: HeaderNavAccessConfig
  docs: boolean
  about: boolean
...
const cloneHeaderNavDefault = (): HeaderNavModulesConfig => ({
  ...
  modelHealth: { ...HEADER_NAV_DEFAULT.modelHealth },
})
...
export function parseHeaderNavModules(
  ...
  try {
    ...
    Object.entries(parsed).forEach(([key, raw]) => {
      ...
      if (key === 'modelHealth') {
        result.modelHealth = parseAccessModule(raw, base.modelHealth)
        return
      }
